...
describe('<CannabisProductProfile />', () => {
  ...
  it("shouldn't throw error if data is specified but follows or likes are not specified", () => {
    try {
      shallow(
        <MemoryRouter>
          <CannabisProductProfile data={data} />
        </MemoryRouter>
      );
    } catch (error) {
      expect(error).toBeUndefined();
  ...
  it("shouldn't render Buy Section if shopifyProduct is not specified", () => {
    const renderedComponent = shallow(
      <MemoryRouter>
        <CannabisProductProfile data={data} follows={follows} likes={likes} />
      </MemoryRouter>
    );
    expect(
      renderedComponent.find('.cannabisProductProfile__buySection').length
    ).toEqual(0);
  });
  ...
  it("shouldn't require auth to follow or like the product when currentUser is specified", () => {
    const renderedComponent = shallow(
      <MemoryRouter>
        <CannabisProductProfile data={data} currentUser={{}} />
      </MemoryRouter>
    );
  ...
  it('should require auth to follow or like the product when currentUser is not specified', () => {
    ...
    expect(renderedComponent.find('RequireAuth').length).toEqual(3);
  });
